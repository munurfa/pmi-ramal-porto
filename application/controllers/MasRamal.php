...
class MasRamal extends CI_Controller {
	...
	public function butuh($jenis='a',$komp='WB',$tgl_ramal=null)
	{
        // date('Y-m')
		...
		$data = array(
        				'title' => 'Kebutuhan Darah',
                        'sample' => $all,
        				'allkomdar' => $this->Keluar_M->allkomdar()->result(),
                        'masuk' => $this->Ramal_M->masuk($komp.'%',$tgl_ramal.'%')->row(),
        				'content' => 'peramalan/mas',
        				'jenis' => $jenis,
        				'komp' =>$komp,
        				'tgl_ramal' => $tgl_ramal,
	...
	public function carikeluar($jenis='a',$komp='WB',$tgl_ramal)
    {
    	...
    	$tgl_ramal = ($tgl_ramal=='') ? date_format($date, "Y-m-d") : $tgl_ramal ;
        $this->form_validation->set_rules('tgl_ramal', 'TANGGAL', 'required');
            if ($this->form_validation->run() == FALSE) {
             redirect('butuh/'.$jenis.'/'.$komp.'/'.$tgl_ramal);
         } else {
            $tanggal = $this->input->post('tgl_ramal');
            redirect('butuh/'.$jenis.'/'.$komp.'/'.$tanggal);
        }
        return false;
    }
